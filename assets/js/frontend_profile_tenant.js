...
var FrontendTenantProfile = {
    ...
    initialize: function (bindEventHandlers, manageMode) {
        ...
        $("#phone-number-fb , .phone-number").intlTelInput({
            nationalMode: true,
            utilsScript: GlobalVariables.baseUrl + "assets/ext/intl-tel-input/utils.js", // just for formatting/placeholders etc
            autoPlaceholder: false
        });

        FrontendTenantProfile.manageMode = manageMode;
        FrontendTenantProfile.resetModalInput();
        ...
        $('#select-date').datepicker({
            ...
            onSelect: function (dateText, instance) {
                FrontendTenantProfile.getAvailableHours(dateText);
        ...
        if (bindEventHandlers) {
            FrontendTenantProfile.bindEventHandlers();
        }

        // If the manage mode is true, the appointments data should be
        // loaded by default.
        if (FrontendTenantProfile.manageMode) {
            FrontendTenantProfile.applyAppointmentData(GlobalVariables.appointmentData,
...
//console.log(GlobalVariables);
        ...
        $('#select-provider').change(function () {
            FrontendTenantProfile.getAvailableHours(Date.today().toString('dd-MM-yyyy'));
            FrontendTenantProfile.updateConfirmFrame();
        });
...
//            FrontendTenantProfile.updateServiceDescription($('#select-service').val(), $('#service-description'));
        ...
        $("#waiting-appointment").click(function () { // Click to only happen on announce links
            ...
            $('.hhh').addClass('hidden');
            $('.www').removeClass('hidden');
            $('#waiting-appointment').addClass('hidden');
        ...
        $(document).on("click", '#button-next-2', function (event) {
            // If we are on the first step and there is not provider selected do not continue
            // with the next step.
            if (!$('.hhh').hasClass('hidden') && $('.selected-hour').length == 0) {
                ...
                return;
            } else if (!$('.www').hasClass('hidden') && $('#waiting-hour').val() == '') {
                return;
        ...
        $(document).on("click", '#button-next-3', function (event) {
...

            {
                ...
                formData['customer'] = {
                    ...
                    'first_name': $('#first-name').val(),
                    'email': $('#email').val(),
                    'phone_number': $("#phone-number-fb").intlTelInput("getNumber"),
            ...
            if (!FrontendTenantProfile.validateCustomerForm()) {
                return;
            }
        ...
        $('.button-back').click(function () {
            if (!jQuery.isEmptyObject(GlobalVariables.customerData))
            {
                ...
                $('.www').addClass('hidden');
                $('.hhh').removeClass('hidden');
                $('#waiting-appointment').removeClass('hidden');
            }
        ...
        $('#book-appointment-submit').click(function (event) {
            ...
            if (!$('.hhh').hasClass('hidden'))
            {
                FrontendTenantProfile.registerAppointment();
            } else {
                ...
                var formData = {
                    ...
                    'end_datetime': FrontendTenantProfile.calcEndDatetimeWaiting(),
                    'notes': $('#notes').val(),
                    'id_users_provider': $('#select-provider').val(),
                    'id_services': $('#select-service').val(),
                    'id_users_customer': GlobalVariables.customerData['user_id']
                };
    ...
    getAvailableHours: function (selDate) {
        ...
        var appointmentId = (FrontendTenantProfile.manageMode)
        ...
        var postData = {
            ...
            'manage_mode': FrontendTenantProfile.manageMode,
    ...
    updateConfirmFrame: function () {
        ...
        postData['appointment'] = {
            ...
            'end_datetime': FrontendTenantProfile.calcEndDatetime(),
            'notes': $('#notes').val(),
        ...
        postData['manage_mode'] = FrontendTenantProfile.manageMode;
