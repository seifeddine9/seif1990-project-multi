...
class Providers_Model extends CI_Model {
     ...
     */
    public function get_available_providers($id_tenant) {
        // Get provider records from database.
        $this->db
                ->select('ea_users.*')
                ->from('ea_users')
                ->join('ea_roles', 'ea_roles.id = ea_users.id_roles', 'inner')
				->where('ea_users.id_tenant', $id_tenant)
                ->where('ea_roles.slug', DB_SLUG_PROVIDER);
     ...
     * @return array Returns an array with the providers data.
     */
    public function get_available_providers_customers() {
        // Get provider records from database.
        $this->db
                ->select('ea_users.*')
                ->from('ea_users')
     ...
     * @return int Returns the role id for the provider records.
     */
    public function get_tenant($provider_id) {
        return $this->db->get_where('ea_users', array('id' => $provider_id))->row()->id_tenant;
    }
    /**
     ...
     */
    public function set_setting($setting_name, $value, $provider_id,$id_tenant) {
        $this->db->where(array('id_users' => $provider_id));
		$this->db->where(array('id_tenant' => $id_tenant));
     ...
     */
    private function save_settings($settings, $provider_id, $id_tenant) {
        if (!is_numeric($provider_id)) {
        ...
        if ($this->db->get_where('ea_user_settings', array('id_users' => $provider_id), array('id_tenant' => $id_tenant))
                ->num_rows() == 0) {
            $this->db->insert('ea_user_settings', array('id_users' => $provider_id), array('id_tenant' => $id_tenant));
        }

        foreach($settings as $name=>$value) {
            $this->set_setting($name, $value, $provider_id, $id_tenant);
        }
    }
