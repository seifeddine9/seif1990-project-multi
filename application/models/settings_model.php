...
class Settings_Model extends CI_Model {
     ...
     */
    function get_setting($name,$id_tenant) {
        ...
        $query = $this->db->get_where('ea_settings', array('name' => $name), array('id_tenant' => $id_tenant));
     ...
     */
    function set_setting($name, $value, $id_tenant) {
        if (!is_string($name)) {
            throw new Exception('$name argument is not a string : ' . $name);
        }

        $query = $this->db->get_where('ea_settings', array('name' => $name,'id_tenant' => $id_tenant));
        if ($query->num_rows() > 0) {
            // Update setting
     ...
     */
    public function save_settings($settings, $id_tenant) {
        if (!is_array($settings)) {
            throw new Exception('$settings argument is invalid: '. print_r($settings, TRUE));
        }

        foreach($settings as $setting) {
            $this->db->where(array('name' => $setting['name'],'id_tenant' => $id_tenant ));
     ...
     */
    public function get_settings($id_tenant) {
		$this->db->where('id_tenant', $id_tenant);
