...
class User_Model extends CI_Model {
     ...
     */
    public function get_settings($user_id,$id_tenant) {
        $user = $this->db->get_where('ea_users', array('id' => $user_id), array('id_tenant' => $id_tenant))->row_array();
    ...
    public function check_login($email, $password) {
        ...
        $user_data = $this->db
                ->select('ea_users.id AS user_id, ea_users.email AS user_email, '
                        ...
                        . 'ea_users.first_name  , ea_users.phone_number, ea_users.id_tenant')
                ->from('ea_users')
   ...
   /**
    ...
    public function get_tenants($key){
        
        if($key!=''){
        $where_clause = 'SELECT * FROM ea_users WHERE id = id_tenant AND (first_name LIKE "%' . $key . '%" OR ' .
            ...
            $where_clause='SELECT * FROM ea_users WHERE id = id_tenant';
        }
