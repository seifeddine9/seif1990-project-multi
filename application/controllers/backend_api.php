...
class Backend_api extends CI_Controller {
    ...
    public function ajax_save_appointment() {
        try {
            ...
            $service_setting = $this->settings_model->get_setting('company_service', $id_tenant);
            $service_double = $this->settings_model->get_setting('enable_double', $id_tenant);
            ...
            if ($number_services <= $service_setting && $service_double == '1' || $number_services == 0 && $service_double == '0') {
                ...
                if (isset($_POST['appointment_data'])) {
                    $appointment = json_decode(stripcslashes($_POST['appointment_data']), true);
                    $appointment['id_tenant'] = $this->session->userdata('id_tenant');
                    ...
                    if ($this->settings_model->get_setting('confirm_appointment', $id_tenant) === '1') {
                ...
                $company_settings = array(
                    'company_name' => $this->settings_model->get_setting('company_name', $id_tenant),
                    'company_link' => $this->settings_model->get_setting('company_link', $id_tenant),
                    'company_email' => $this->settings_model->get_setting('company_email', $id_tenant)
                );


                // :: add notification RECORD to DATABASE
                $notifications['id_tenant'] = $this->session->userdata('id_tenant');
                ...
                if ($this->settings_model->get_setting('sms_notification', $id_tenant) == '1') {
                    if (!$manage_mode) {
                ...
                try {
                    ...
                    if ($google_sync == TRUE) {
                        $google_token = json_decode($this->providers_model->get_setting('google_token', $appointment['id_users_provider'], $id_tenant));
    ...
    public function ajax_delete_appointment() {
        try {
            ...
            if ($appointment['id_google_calendar'] != NULL) {
                try {
                    $google_sync = $this->providers_model->get_setting('google_sync', $provider['id'], $id_tenant);
    ...
    public function ajax_disable_provider_sync() {
        try {
            ...
            $this->load->model('providers_model');
            $this->load->model('appointments_model');
            $this->providers_model->set_setting('google_sync', FALSE, $_POST['provider_id'], $id_tenant);
            $this->providers_model->set_setting('google_token', NULL, $_POST['provider_id'], $id_tenant);
    ...
    public function ajax_filter_customers() {
        try {
            ...
            $where_clause = '(id_tenant =' . $id_tenant . ' AND (first_name LIKE "%' . $key . '%" OR ' .
            ...
            foreach ($customers as &$customer) {
                $appointments = $this->appointments_model
                        ->get_batch(array('id_users_customer' => $customer['id']), array('id_tenant' => $id_tenant));
    ...
    public function ajax_save_unavailable() {
        try {
            ...
            // Google Sync
            try {
                $google_sync = $this->providers_model->get_setting('google_sync', $unavailable['id_users_provider'], $id_tenant);

                if ($google_sync) {
                    $google_token = json_decode($this->providers_model->get_setting('google_token', $unavailable['id_users_provider'], $id_tenant));
    ...
    public function ajax_delete_unavailable() {
        try {
            ...
            try {
                $google_sync = $this->providers_model->get_setting('google_sync', $provider['id'], $id_tenant);
                if ($google_sync == TRUE) {
                    $google_token = json_decode($this->providers_model->get_setting('google_token', $provider['id'], $id_tenant));
                    $this->load->library('google_sync');
                    $this->google_sync->refresh_token($google_token->refresh_token);
    ...
    public function ajax_get_waiting() {
        try {
            ...
            if (isset($_POST['dates'])) {
                ...
                $waiting_list = $this->waiting_model->get_batch_filter($date_debut, $date_fin, $id_tenant);
            } else {
                $waiting_list = $this->waiting_model->get_batch(array('id_tenant' => $id_tenant));
            }
    ...
    public function ajax_get_appointment() {
        try {
            ...
            if (isset($_POST['dates'])) {
                ...
                $date_fin = $dates['date_fin'];
                $where = ' start_datetime between ' . $date_debut . ' AND ' . $date_fin . ' ';
                $appointment_list = $this->appointments_model->get_batch_filter($date_debut, $date_fin, $id_tenant);
            } else {
                $appointment_list = $this->appointments_model->get_batch(array('id_tenant' => $id_tenant));
            }
    ...
    public function ajax_get_notification() {
        try {
            ...
            $notification_list = $this->notifications_model->get_batch(array('id_tenant' => $id_tenant));
    ...
    public function ajax_filter_dates() {
        try {
            ...
            $date_debut = $dates['date_debut'];
            $date_fin = $dates['date_fin'];
            $all = $this->appointments_model->get_count_filter($date_debut, $date_fin, $id_tenant);
            $all_price = $this->appointments_model->get_count_filter_price($date_debut, $date_fin, $id_tenant);
            $confirmed = $this->appointments_model->get_count_confirmed_filter($date_debut, $date_fin, $id_tenant);
            $projected = $this->appointments_model->get_count_projected_filter($date_debut, $date_fin, $id_tenant);
    ...
    public function ajax_filter_services() {
        try {
            ...
            $this->load->model('services_model');
            $key = $this->db->escape_str($_POST['key']);
            $where = '(id_tenant=' . $id_tenant . ' AND (name LIKE "%' . $key . '%" OR duration LIKE "%' . $key . '%" OR ' .
    ...
    public function ajax_filter_service_categories() {
        try {
            ...
            $this->load->model('services_model');
            $key = $this->db->escape_str($_POST['key']);
            $where = '(id_tenant = ' . $id_tenant . ' AND (name LIKE "%' . $key . '%" OR description LIKE "%' . $key . '%"))';
    ...
    public function ajax_filter_admins() {
        try {
            ...
            $key = $this->db->escape_str($_POST['key']);
            $where = '(id_tenant=' . $id_tenant . ' AND (first_name LIKE "%' . $key . '%" OR last_name LIKE "%' . $key . '%" ' .
    ...
    public function ajax_save_settings() {
        try {
            if ($_POST['type'] == SETTINGS_SYSTEM) {
                ...
                $this->settings_model->save_settings($settings, $id_tenant);
    ...
    public function ajax_get_google_calendars() {
        try {
            ...
            $google_sync = $this->providers_model->get_setting('google_sync', $_POST['provider_id'], $id_tenant);
            if ($google_sync) {
                $google_token = json_decode($this->providers_model->get_setting('google_token', $_POST['provider_id'], $id_tenant));
                $this->google_sync->refresh_token($google_token->refresh_token);
    ...
    public function ajax_select_google_calendar() {
        try {
            ...
            $id_tenant = $this->session->userdata('id_tenant');
            $this->load->model('providers_model');
            $result = $this->providers_model->set_setting('google_calendar', $_POST['calendar_id'], $_POST['provider_id'], $id_tenant);
